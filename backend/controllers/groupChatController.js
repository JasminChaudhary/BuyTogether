...
// @route   POST /api/group-chat/:groupId/messages
...
export const sendMessage = async (req, res) => {
    try {
        ...
        const populatedMessage = await GroupChat.findById(chatMessage._id)
        ...
        const members = await GroupMember.find({ group: groupId, tokenStatus: "PAID" });
        const notificationsToCreate = members
            .filter(member => member.buyer.toString() !== senderId.toString())
            .map(member => ({
                user: member.buyer,
                type: "PROPERTY_GROUP_MESSAGE",
...
// @route   GET /api/group-chat/:groupId/messages
// @access  Private (Buyer/Admin)
